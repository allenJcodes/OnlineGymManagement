    ...
    <div class="container pt-14">
        <div
            ...
            <form action="{{ route('updateItem', $item->id) }}" method="POST">
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <input value="{{ $item->quantity }}" type="number" name="quantity" id="floating_password"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " required />
                ...
                <div class="grid md:grid-cols-2 md:gap-6">
                    <div class="relative z-0 w-full mb-6 group">
                        ...
                        < <label for="equipment_type" class="block pb-2 text-sm font-medium text-gray-900 dark:text-white">
                            Equipment
                            Type</label>
                            <select id="equipment_type" name="equipment_type_id"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option value="" @if (!$item->equipment_type_id) selected="selected" @endif
                                    disabled>
                                    Select
                                    type of equipment</option>
                                @foreach ($types as $type)
                                    <option value="{{ $type->id }}"
                                        @if ($type->id == old('equipment_type', $item->equipment_type_id)) selected="selected" @endif> {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                    </div>
                    <div class="relative z-0 w-full mb-6 group">
                        <input value="{{ $item->purchase_date }}" type="date" name="purchase_date"
